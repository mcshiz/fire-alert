...
class ListRecipients extends React.Component {
    ...
    render() {
        let Recipients = this.props.recipients.map((recipient, idx) => (
            <li className='recipient-item ' key={idx}>
                <form className="form-inline" onSubmit={this.props.applyEdits.bind(this, recipient)} method='put' name={idx}>
                    ...
                    <div className="col-xs-12">
                        <div className="form-group">
                            <button type='submit' disabled={true} className='btn btn-primary submit' >Save <i className='fa fa-check'></i></button>
                            <button type='button' className='btn btn-danger remove' onClick={this.props.deleteRecipient.bind(this, recipient)}>Delete <i className='fa fa-trash'></i></button>
                        </div>
                    </div>
                </form>
            </li>
